...
const Chart = () => {
  ...
  useEffect(() => {
    if (location.state?.data) {
      ...
      setListHeart(
        location.state.data.data?.heartRate
          ? location.state.data.data?.heartRate
          : []
      );
    }
  ...
  const onChange = async (value: string) => {
    ...
    if (history) {
      setHistory(value);
      try {
        const response = await coordinateAPI.getCoordinates({ name: value });
        if (response.success) {
          setListHeart(
            response.data?.heartRate ? response.data?.heartRate : []
          );
        }
      } catch (error) {}
    }
  ...
  useEffect(() => {
    (async () => {
      ...
      if (response.success) {
        const items = response.data.histories.map(
          ...
          <ResponsiveContainer width="100%" height={250}>
            <LineChart margin={{ top: 30, bottom: 5 }}>
              ...
              <Line
                ...
                dataKey="heartRate"
                name="Nhịp tim"
                data={listHeart.map((item: any) => {
                  const hours =
                    dayjs(item.created).hour() +
                    dayjs(item.created).minute() / 60;
                  return {
